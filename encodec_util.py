...
@cache
def _load_model(bandwidth=6.0, device="cpu"):
...
def decode(codes: Tensor, bandwidth=6.0, device="cpu"):
    """
    Args:
        codes: (b q t)
    """
...
@torch.inference_mode()
def encode(wav: Tensor, sr: int, bandwidth=6.0, device="cpu"):
    """
    Args:
        wav: (t)
        sr: int
...
def encode_from_file(path, bandwidth=6.0, device="cpu"):
...
def decode_files(folder, suffix=".qnt.pt"):
    ...
    for path in tqdm(paths):
        ...
        if out_path.exists():
            continue
        fi = rearrange(torch.load(path).squeeze(0).cpu(), "q t -> t q")
